...
bool CopyFileOptimized(const std::string& src, const std::string& dest) {
    try {
        ...
        cerr << "copy file failed: " << e.what() << endl;
        return false;
    }
}
...
bool AddStartupToAutoexec(const std::string& autoexecPath) {
    try {
        ...
        cerr << "can't write autoexec: " << e.what() << endl;
        return false;
    }
}
...
bool InitSDL() {
    ...
    if (Mix_Init(MIX_INIT_MP3) != MIX_INIT_MP3) {
        cerr << "SDL_mixer faild: " << Mix_GetError() << endl;
        SDL_Quit();
        return false;
    }
    if (Mix_OpenAudio(22050, MIX_DEFAULT_FORMAT, 2, 1024) < 0) {
        cerr << "Audio failed: " << Mix_GetError() << endl;
        Mix_Quit();
        SDL_Quit();
        return false;
    }
...
void PlayBGMFromMemory(UINT resourceID) {
    ...
    if (!hRes) {
        cerr << "can't find resource" << endl;
        return;
    }
    HGLOBAL hData = LoadResource(NULL, hRes);
    if (!hData) {
        cerr << "can't load resource" << endl;
        return;
    }
    DWORD dwSize = SizeofResource(NULL, hRes);
    void* pData = LockResource(hData);
    if (!pData) {
        cerr << "locked resource failed" << endl;
        return;
    }
    SDL_RWops* rw = SDL_RWFromConstMem(pData, dwSize);
    if (!rw) {
        cerr << " RWops failed: " << SDL_GetError() << endl;
        return;
    }
    Mix_Music* music = Mix_LoadMUS_RW(rw, 1);  // 自动释放 rw
    if (!music) {
        cerr << "Can't load music: " << Mix_GetError() << endl;
        return;
    }
...
void AppendToFile(const string& filePath, const string& content) {
    ...
    if (file.is_open()) {
        ...
        MessageBox(NULL, L"Some went wrong", L"错误", MB_ICONERROR);
    }
}
...
int main() {
    ...
    fs::path target_path = L"Counter-Strike Global Offensive\\game\\csgo\\cfg\\Sqaure";
    if (current_path.filename() != target_path.filename()) {
        MessageBoxW(NULL, L"The running directory is incorrect. This program should be under game/csgo/cfg/Sqaure", L"错误", MB_OK | MB_ICONERROR);
    }
    else {
      ...
      //  wstring messageG = L"检测到 GPU: " + gpuInfo;
       ...
       // system("color 0F");
        ...
        if (checkFile.good()) {
            ...
            MessageBox(NULL, L"Saved failed。", L"错误", MB_ICONERROR);
        }
        // 询问是否打开模组市场
        int modResponse = MessageBox(NULL, L"Open ModMarket?", L"Mod market", MB_YESNO | MB_ICONQUESTION);
        if (modResponse == IDYES) {
            ...
            if (CreateProcess(L"ModsMarket.exe", NULL, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi)) {
                ...
                MessageBox(NULL, L"can't open market！", L"错误", MB_OK | MB_ICONERROR);
            }
        }
        ...
        cout << "All files copied completed！\n";
        ...
        int autoexecResponse = MessageBox(NULL, L"Auto load config in autoexec？", L"启动项", MB_YESNO | MB_ICONQUESTION);
        if (autoexecResponse == IDYES) {
            if (AddStartupToAutoexec("..\\autoexec.cfg")) {
                MessageBox(NULL, L"OK", L"提示", MB_OK | MB_ICONINFORMATION);
            }
            else {
                MessageBox(NULL, L"failed!", L"错误", MB_OK | MB_ICONERROR);
            }
        }

        MessageBox(NULL, L"Now you can close this program", L"tips", MB_OK | MB_ICONINFORMATION);
